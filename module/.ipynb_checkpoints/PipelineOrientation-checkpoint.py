...
class Pipeline:
    ...
    def f1_std(self):
        result = {}
        for key in self.scores.keys():
            result[key] = self.scores[key]["f1"].std()
        return result
...
class Preprocessing(PipelineObject):
    ...
    '''
    def __init__(self, raw="local", processes=[]):
...
def train(name, participant, clf, train_x, train_y, test_x, test_y):
    ...
    threshold_accuracy = (precision, recall, thresholds)
    ...
    for t in range(0, 50):
        ...
        threshold_accuracy[threshold] = (a, p, r)
    '''
    
    conMatrix = metrics.confusion_matrix(test_y, predict_y)
    f1Score = metrics.f1_score(test_y, predict_y, pos_label=1)#"relax")
    ...
    videoScores = {}
    videof1Score = {}
    return name, participant, score, conMatrix, f1Score, videoScores, videof1Score, importances, report, model.classes_, test_y, predict_y, threshold_accuracy
...
class Classification(PipelineObject):
    ...
    def execute(self):
        ...
        for future in futures:
            name, participant, score, conMatrix, f1Score, videoScores, videof1Score, importances, report, classes_, test_y, predict_y, threshold_accuracy = future.result()
            ...
            self.results[name][participant]["threshold_accuracy"] = threshold_accuracy
...
def evaluate(features, sensors, lpf=LowPassFilter(hrange=0.4, hbias=0.4), window_size=10, window_step=5.0, remove_seconds=30, shuffle_videos=True, labelConfig=labeling.SimpleConfig()):
    pipes = []
    for i in range(10):
        pipe = Pipeline(Preprocessing("ned", processes=[
    ...
    for key in keys:
        ...
        f1stdPerCLF = np.mean(list(map(lambda p: p.f1_std()[key], pipes)))
        table.append([key, "%0.2f +- %0.2f" % (accPerCLF, stdPerCLF), "%0.2f +- %0.2f" % (f1sPerCLF, f1stdPerCLF)])
    #display(HTML(tabulate.tabulate(table, tablefmt='html')))
    return table[1:]
    #print(table[1:])
